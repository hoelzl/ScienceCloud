...
(defvar *program* #'cloud-robot-prog)
...
(defun current-algorithm ()
  (ad-algorithm (aref *algorithms* 0)))
...
(defun make-new-environment (&optional (type *environment-type*)
  ...
  (ecase type
    ...
    ((:large) (if complexp
                  (make-cloud-env-5)
                  (make-cloud-env-4)))
    ((:maze :labyrinth) (make-cloud-env-6))))
          ...
          (ecase *environment-type*
            ((:small)
             (if (eq *exploration-strategy* :random)
                 (if *use-complex-environment*  500000 500000)
                 (if *use-complex-environment*  500000 500000)))
            ((:medium)
             (if (eq *exploration-strategy* :random)
                 (if *use-complex-environment* 10000000 10000000)
                 (if *use-complex-environment*  2500000  2500000)))
        ...
        (list
         ...
         (list 'hordq-a-0 
               :algorithm (make-instance 'ahq:<hordq>
                            :features *cloud-featurizer-0*
                            :lrate hordq-learning-rate
                            :discount hordq-discount)
               :bucket-function *cloud-bucket-function-0*
               :test #'equalp)
         (list 'hordq-a-1 
               :algorithm (make-instance 'ahq:<hordq>
                            :features *cloud-featurizer-1*
                            :lrate hordq-learning-rate
                            :discount hordq-discount)
               :bucket-function *cloud-bucket-function-1*
               :test #'equalp)
         (list 'hordq-a-2 
               :algorithm (make-instance 'ahq:<hordq>
                            :features *cloud-featurizer-2*
                            :lrate hordq-learning-rate
                            :discount hordq-discount)
               :bucket-function *cloud-bucket-function-2*
               :test #'equalp)
         (list 'hordq-a-3 
               :algorithm (make-instance 'ahq:<hordq>
                            :features *cloud-featurizer-3*
                            :lrate hordq-learning-rate
                            :discount hordq-discount)
               :bucket-function *cloud-bucket-function-3*
               :test #'equalp)))
  (values))
...
(defun evaluate-performance (&key (algorithm-names *algorithm-names*)
                                  (output-directory
                                   (merge-pathnames
                                    ...
                                    (asdf:system-definition-pathname :cloud)))
                                  (gnuplot-file-prefix)
