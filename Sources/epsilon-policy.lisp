...
(defmethod initialize-instance :after ((policy <epsilon-policy>) &rest initargs &key)
      ...
      (let* ((choices (alisp:js-choices state))
        ...
        (or cached-value
            (let ((dist (prob:make-unif-dist-over-set choices)))
              ...
              ;; (force-format t "~&>>Random choice ~A ~A" *random-choice-counter* choices)
              dist)))
